...
function addProizvod($katID, $ime, $cena, $url)
{
    global $conn;
    $rarray = array();
    $errors = "";
    if (checkIfLoggedIn()) {
        ...
        if ($errors == "") {
            $stmt = $conn->prepare("INSERT INTO proizvod (katID, ime,  cena, url) 
        VALUES (?, ?,  ?, ?)");
            $stmt->bind_param('isis', $katID, $ime,  $cena, $url);
